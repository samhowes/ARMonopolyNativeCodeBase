...
NSString *GetARMLaunchImageName()
{
    // The requirement is for iOS 7
    NSMutableString *imageName = [NSMutableString stringWithString:@"LaunchImage-700"];
    ...
    else if (([[UIScreen mainScreen] bounds].size.height - ( double )568 ) < DBL_EPSILON)
    {
        // If we are on a widescreen device
        [imageName appendString:@"-568h"];
    }
    
    // the final scaleModifier will be added automagically
    return imageName;
}
...
void ShowSplashScreen(UIWindow* window)
{
    ...
    NSLog(@"ARM-------> Setting ARM SplashScreen");
    UIImage *armSplashImage =[UIImage imageNamed:GetARMLaunchImageName()];
    [_splash setImage:armSplashImage];
